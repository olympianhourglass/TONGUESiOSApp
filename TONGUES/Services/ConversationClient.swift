...
enum ConversationClient {
    ...
    private static func buildSystemPrompt(context: Context) -> String {
        ...
        lines.append("")
        lines.append("YOUR PERSONA — this is the MOST IMPORTANT instruction and overrides any default friendliness. Commit to it fully so it is unmistakable in every single turn:")
        lines.append(context.tone.replyPersona)
        lines.append("")
        ...
        lines.append("• Keep turns short — Beginners: 1 short sentence, Intermediate: 1–2, Advanced: 2–3 — though your persona may run a touch longer when it genuinely calls for it (e.g. WARM scaffolding a reply, WITHERING landing a quip). Never turn it into a lecture.")
        lines.append("• Stay fully in your persona above; use the target language's natural, colloquial register.")
