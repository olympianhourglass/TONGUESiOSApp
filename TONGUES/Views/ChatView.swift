...
struct ChatView: View {
    ...
    // Mandarin only: reveals the pinyin (romanization) under Chinese text in
    // the bubbles and suggestion chips. Off by default so the learner reads the
    // characters first; the eye toggle in each assistant bubble flips it.
    @State private var revealPinyin = false

    // True when the active conversation is a Chinese variety, so the pinyin
    // reveal affordance (eye toggle) is offered and romanization stays hidden
    // until asked for.
    private var isChineseConversation: Bool {
        isChineseLanguage(vm.conversation?.language ?? selectedLanguage)
    }

    // Whether romanization should render right now. Non-Chinese languages keep
    // showing it always (unchanged); Chinese hides it behind the eye toggle.
    private var showsRomanization: Bool {
        isChineseConversation ? revealPinyin : true
    }
    ...
    private func userBubble(_ message: ConversationMessage) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            // assistant bubble shows, just tinted for the dark bubble. Hidden
            // behind the pinyin toggle for Chinese (see showsRomanization).
            if showsRomanization, let translit = message.transliteration, !translit.isEmpty {
                Text(translit)
    ...
    private func assistantBubble(_ message: ConversationMessage) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 14) {
                ...
                // Mandarin only: reveal/hide the pinyin under the Chinese text
                // in every bubble and hint. Sits just before the save button.
                if isChineseConversation {
                    Button {
                        Haptics.light()
                        withAnimation(.easeInOut(duration: 0.2)) { revealPinyin.toggle() }
                    } label: {
                        Image(systemName: revealPinyin ? "eye.slash" : "eye")
                            .font(.system(size: MacLayout.f(16)))
                            .foregroundStyle(.secondary)
                    }
                    .buttonStyle(.plain)
                    .accessibilityLabel(revealPinyin ? L("Hide pinyin") : L("Show pinyin"))
                }
    ...
    private func suggestionChip(_ suggestion: ConversationClient.SuggestedReply) -> some View {
        Button {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                if showsRomanization, let translit = suggestion.transliteration, !translit.isEmpty {
                    Text(translit)
                        .font(.custom("NeueHaasDisplay-Light", size: MacLayout.f(11)))
                        .italic()
                        .foregroundStyle(.secondary)
